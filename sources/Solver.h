...
void V_constrain(VerletObject *obj, Vector2 center, double radiusConstr,
                 double radiusSmall) {
  ...
  if (dist > (radiusConstr - radiusSmall)) {
    const Vector2 n =
        Vector2Scale(Vector2Normalize(to_obj), radiusConstr - radiusSmall);
    obj->pos = Vector2Add(center, n);
  }
}

void V_checkCollide(VerletObject *curr, VerletObject *arr[], int numObjs,
                    double radius) {
  ...
  for (int i = 0; i < numObjs; i++) {
    ...
    if ((curr != arr[i]) &&
        (CheckCollisionCircles(curr->pos, radius, arr[i]->pos, radius))) {
      ...
      const Vector2 n =
          Vector2Scale(Vector2Subtract(curr->pos, arr[i]->pos), 1 / dist);

      // reduce scale factor when adding.
      const double delta = (radius * 2 - dist) * V_Dampening;

      // add to the vector in the direction of the vector normalized, scaled by
      // the scale factor
...
if(dist > (SCREEN_HEIGHT / 2) - radius) {
    ...
    //n is the distance from center of circle to outer circle minus radius of
small circle const Vector2 n = Vector2Scale(Vector2Normalize(to_obj),
(SCREEN_HEIGHT / 2) - radius); v.pos = Vector2Add(V_BigCircle, n);
